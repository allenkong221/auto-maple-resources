"""A collection of all commands that a mage (I/L) can use to interact with the game."""
...
class Buff(Command):
    ...
    def main(self):
        buffs = [Key.INFINITY, Key.HOLY_SYMBOL, Key.SHARP_EYES]
        now = time.time()
...
class Lightning(Command):
    ...
    def main(self):
        ...
        for _ in range(self.repetitions):
            press(Key.LIGHTNING, self.attacks, up_time=0.05)
        key_up(self.direction)
        if self.attacks > 2:
            time.sleep(0.5)
        else:
...
class Elquines(Command):

    def main(self):
        press(Key.ELQUINES, 3)
...
class SpiritSnow(Command):

    def main(self):
        press(Key.SPIRIT_OF_SNOW, 3)

class Blizzard(Command):

    def main(self):
        press(Key.BLIZZARD, 3)
...
class ErdaFountain(Command):
    ...
    def main(self):
        ...
        if self.direction == 'up':
            key_down(self.direction)
            time.sleep(0.05)
        press(Key.ERDA_FOUNTAIN, num_presses)
        key_up(self.direction)